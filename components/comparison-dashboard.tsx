...
export function ComparisonDashboard() {
  ...
  const bestRegion =
    comparisonData.length > 0
      ? comparisonData.reduce(
          (max, current) => (current.companies > max.companies ? current : max),
          comparisonData[0]
        )
      : null;
  ...
  const totalInvestment = comparisonData.reduce(
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Bandingkan kinerja ekonomi kreatif antar wilayah dan periode
            </p>
          </div>
        </div>
        <Card>
              ...
              <div className="grid grid-cols-3 gap-2">
                {availableYears.map((year) => (
                  <Button
                    ...
                    disabled={
                      !selectedYears.includes(year) && selectedYears.length >= 3
                    }
                  >
                    {year}
                  </Button>
          ...
          <CardContent className="p-12">
            <div className="text-center">
              <p className="text-gray-600 mb-4">
                Pilih wilayah dan tahun untuk melihat perbandingan
              </p>
              <p className="text-sm text-gray-500">
                Minimal pilih 1 wilayah dan 1 tahun
              </p>
            </div>
          </CardContent>
        </Card>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-blue-900">
                        {bestRegion
                          ? bestRegion.region.replace("Kabupaten ", "Kab. ")
                          : "-"}
                      </p>
                      <p className="text-sm text-blue-700">
                        {bestRegion
                          ? `${bestRegion.companies.toLocaleString()} pelaku usaha`
                          : "-"}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-green-900">
                        {highestGrowth && highestGrowth.growth > 0
                          ? `+${highestGrowth.growth.toFixed(1)}%`
                          : "-"}
                      </p>
                      <p className="text-sm text-green-700">
                        {highestGrowth && highestGrowth.growth > 0
                          ? `${highestGrowth.region.replace(
                              "Kabupaten ",
                              "Kab. "
                            )} ${highestGrowth.year}`
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-purple-900">
                        {formatCurrency(bestRegion ? bestRegion.investment : 0)}
                      </p>
                      <p className="text-sm text-purple-700">
                        Pada wilayah dan tahun terbaik
                      </p>
                    </div>
