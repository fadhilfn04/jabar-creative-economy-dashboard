...
import type { RankingData } from "@/lib/unified-database-service"

type TabType = 'investment' | 'workforce' | 'projects'

export function OptimizedRankingAnalysis() {
  const [activeTab, setActiveTab] = useState<TabType>('investment')
  const [selectedYear, setSelectedYear] = useState<number>(2024)
  const [availableYears, setAvailableYears] = useState<number[]>([])
  
  // Data states
  const [investmentData, setInvestmentData] = useState<RankingData[]>([])
  const [workforceData, setWorkforceData] = useState<RankingData[]>([])
  const [projectData, setProjectData] = useState<RankingData[]>([])
  ...
  useEffect(() => {
    const fetchYears = async () => {
      try {
        const years = await UnifiedDatabaseService.getAvailableYears()
  ...
  const fetchInvestmentData = async (page: number = 1) => {
    try {
      ...
      const result = await UnifiedDatabaseService.getInvestmentRanking({
        year: selectedYear,
        page,
        limit: pageSize
      })
  ...
  const fetchWorkforceData = async (page: number = 1) => {
    try {
      ...
      const result = await UnifiedDatabaseService.getWorkforceRanking({
        year: selectedYear,
        page,
        limit: pageSize
      })
  ...
  // For projects, we'll use investment data as they're related
  ...
  const exportData = (data: RankingData[], filename: string) => {
    try {
      let csvContent = ''
      let headers = []
      
      if (activeTab === 'investment') {
        headers = ['Peringkat', 'Kabupaten/Kota', 'Jumlah Proyek', 'Tambahan Investasi (US$)', 'Tambahan Investasi (Rp)', 'Persentase (%)']
        csvContent = [
          headers.join(','),
          ...data.map((row: RankingData) => [
            row.rank,
            `"${row.kabupaten_kota}"`,
            row.project_count,
            row.investment_usd || 0,
            row.investment_idr || 0,
            row.percentage
          ].join(','))
        ].join('\n')
      } else if (activeTab === 'workforce') {
        headers = ['Peringkat', 'Kabupaten/Kota', 'Jumlah Proyek', 'Jumlah Tenaga Kerja', 'Persentase (%)']
        csvContent = [
          headers.join(','),
          ...data.map((row: RankingData) => [
            row.rank,
        ...
        headers = ['Peringkat', 'Kabupaten/Kota', 'Jumlah Proyek', 'Tambahan Investasi (Rp)', 'Persentase (%)']
        csvContent = [
          headers.join(','),
          ...data.map((row: RankingData) => [
            row.rank,
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-gray-100">
              ...
              <TableHead className="font-medium text-gray-700 w-24">Persentase</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {investmentData.length === 0 ? (
              ...
              <>
                {investmentData.map((row) => (
                  <TableRow key={`${row.kabupaten_kota}-${row.rank}`} className="border-gray-100 hover:bg-gray-50">
                    <TableCell className="text-center">
                      <Badge variant="outline" className="font-mono">
                        {row.rank}
                      </Badge>
                    </TableCell>
                    <TableCell className="font-medium text-gray-900">{row.kabupaten_kota}</TableCell>
                    <TableCell className="font-medium text-blue-600">{row.project_count.toLocaleString()}</TableCell>
                    <TableCell className="font-medium text-green-600">
                      {formatCurrencyUSD(row.investment_usd || 0)}
                    </TableCell>
                    <TableCell className="font-medium text-green-600">
                      {formatCurrencyIDR(row.investment_idr || 0)}
                    </TableCell>
                    <TableCell className="text-center">
                ...
                <TableRow className="border-t-2 border-gray-300 bg-gray-50 font-bold">
                  ...
                  <TableCell className="font-bold text-green-700">
                    {formatCurrencyUSD(investmentData.reduce((sum, item) => sum + (item.investment_usd || 0), 0))}
                  </TableCell>
                  <TableCell className="font-bold text-green-700">
                    {formatCurrencyIDR(investmentData.reduce((sum, item) => sum + (item.investment_idr || 0), 0))}
                  </TableCell>
                  <TableCell className="text-center">
              ...
              <>
                {workforceData.map((row) => (
                  <TableRow key={`${row.kabupaten_kota}-${row.rank}`} className="border-gray-100 hover:bg-gray-50">
                    ...
                    <TableCell className="font-medium text-purple-600">{(row.worker_count || 0).toLocaleString()}</TableCell>
                    <TableCell className="text-center">
                ...
                <TableRow className="border-t-2 border-gray-300 bg-gray-50 font-bold">
                  ...
                  <TableCell className="font-bold text-purple-700">
                    {workforceData.reduce((sum, item) => sum + (item.worker_count || 0), 0).toLocaleString()}
                  </TableCell>
                  <TableCell className="text-center">
            ...
            {projectData.length === 0 ? (
              ...
              <>
                ...
                <TableRow className="border-t-2 border-gray-300 bg-gray-50 font-bold">
                  ...
                  <TableCell className="font-bold text-green-700">
                    {formatCurrencyIDR(projectData.reduce((sum, item) => sum + (item.investment_idr || 0), 0))}
                  </TableCell>
                  <TableCell className="text-center">
