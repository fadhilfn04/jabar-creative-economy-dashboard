...
export function OptimizedDatabaseMetrics() {
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        setLoading(true)
        setError(null)
        const data = await UnifiedDatabaseService.getDashboardMetrics()
        setMetrics(data)
      } catch (err) {
  ...
  const metricsData = [
    ...
    {
      title: "Persentase EKRAF",
      value: loading ? "..." : `${metrics.ekrafPercentage.toFixed(1)}%`,
      change: "+2.1%",
